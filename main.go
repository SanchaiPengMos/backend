...
func (h *UserHandler) booking(c echo.Context) (err error) {
	...
	fmt.Println("1")
	fmt.Println("Usewqr_id", data.User_id)
	fmt.Println("Create_from", data.Create_from)
	fmt.Println("Create_to", data.Create_to)
	fmt.Println("Room_id", data.Room_id)
	//เพิ่มข้อมูล user
